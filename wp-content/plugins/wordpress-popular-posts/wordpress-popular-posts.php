...
<!-- End Wordpress Popular Posts v<?php echo $this->version; ?> -->
		...
		function get_popular_posts($instance, $return = false) {
			...
			if ( !is_array($mostpopular) || empty($mostpopular) ) { // no posts to show
				...
				foreach($mostpopular as $p_index => $p) {
					...
					if ($instance['markup']['custom_html']) { // build custom layout
						...
						$content .= "<li class='rc-item'>{$thumb}<a href=\"{$permalink}\" title=\"{$title}\" class=\"wpp-post-title right-column-link\" target=\"".$this->user_ops['tools']['link']['target']."\">{$title_sub}</a> {$excerpt}<span class=\"post-stats\">{$stats}</span>{$rating}</li>" . "\n";
						
						// INsert FB social plugin after second post
						if ($p_index == 3) 
						{
							//echo '<iframe src="//www.facebook.com/plugins/likebox.php?href=https%3A%2F%2Fwww.facebook.com%2Fsupercraycray&amp;width&amp;height=290&amp;colorscheme=light&amp;show_faces=true&amp;header=true&amp;stream=false&amp;show_border=true&amp;appId=260709560755238" scrolling="no" frameborder="0" style="border:none; overflow:hidden; height:290px;" allowTransparency="true"></iframe>';
						}
					}
				}
