		...
		function get_popular_posts($instance, $echo = true) {		
			...
			if ( !is_array($mostpopular) || empty($mostpopular) ) {
				...
				foreach ($mostpopular as $wppost) {					
					...
					if ($instance['thumbnail']['active'] && $this->thumb) {
						...
						if (!function_exists('get_the_post_thumbnail')) { // if the Featured Image is not active, show default thumbnail
							...
							if (has_post_thumbnail( $the_ID )) { // if the post has a thumbnail, get it
								//the_post_thumbnail('featured-image', array('class' => 'featured-full-width-top'))
								//echo 'here';
								$thumb = "<a href=\"".get_permalink($the_ID)."\" title=\"". $title_attr ."\" >" . get_the_post_thumbnail($the_ID, 'half-landscape', array('class' => 'wpp-thumbnail', 'alt' => $title_attr, 'title' => $title_attr) ) . "</a>";
					...
					$data = array(
						'title' => '<a href="'.get_permalink($the_ID).'" class="right-column-link" title="'. $title_attr . '"><span class="wpp-post-title">'. $tit .'</span></a>',
					...
					if ($instance['markup']['custom_html']) {
						...
						$content .= '<li class="rc-item">'. $thumb .'<a href="'. get_permalink($the_ID) .'" class="right-column-link" title="'. $title_attr .'"><span class="wpp-post-title">'. $tit .'</span></a>'. $post_content .' '. $stats . $rating .'</li>' . "\n";
					}
				}			
